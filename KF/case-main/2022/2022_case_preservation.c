...
void setup() {
  ...
  if(motor_test && cds_test){   // 「光を検知してモータで閂を抜く」試験をするときは 
                                //  prepair_limit だけ待機してからシーケンスを始める
    delay(prepair_limit);
  }
  ...
  if(fall_speed_read){
    ...
    unsigned long xxx;
    EEPROM.get(900,xxx);
    Serial.print("cds_check millis from the reset : ");
    Serial.println(xxx);
    while(1);
  }
  
  if ((! dps.begin_SPI(DPS310_CS) )&&ARLISS) { //SPI通信ができていなかったら無限ループ．ｵﾜﾘ
...
void loop() {
  ...
  if(!ARLISS){                   // 本番じゃないならテストシーケンスに入れる
    if(EEPROM.read(test_sequence_end) == 2){
      Serial.println("50 == 1 : This test sequence is done");
      EEPROM.write(test_sequence_end_2,1);
      EEPROM.write(test_sequence_end,0);
      while(1);
    }
    else{
      if(!frag){
        Serial.println("Sensor test starts!");
        frag = true;
      }
    }
    if(cds_test){
      ...
      //Serial.println("End of cds_test");
      //EEPROM.update(cds_test_done,1);
    }
    if(motor_test){ // 3秒ずつON・OFFになる　2回繰り返す
      digitalWrite(MOTOR_PIN,HIGH);
      delay(10);
      //Serial.println("End of motor test");
    ...
    if(airP_test){ // カルマンフィルタ適用なしの生気圧・生温度から高度を出力
    ...
    //Serial.println("End of the sequence");
    //EEPROM.update(test_sequence_end,1);
    return;
  }
  ...
  if(!Released){
    ...
    */
    ...
    // カルマンフィルタ使用時の初期条件をセンサ値から取得
    Kalman_ic();
  
    if(ifKalman && Released){ // カルマンフィルタを使うなら動く
...
void Final_Judge(){
  ...
  if((Released and Height_frag) or (Released and Time_frag)){
    MotorGo = true;
...
void Kalman_for_T_and_p(int count){//count = 1000 ???
