...
async def takeoff_and_kill(takeoff_altitude, stay_arm_time, arm_time, takeoff_time, takeoff_gap_time):
    await pixhawk.hold()
    await pixhawk.wait_arm_open(light)
    await asyncio.sleep(stay_arm_time)
    await pixhawk.arm()
    ...
    await pixhawk.goto_location(pixhawk.latitude_deg, pixhawk.longitude_deg, pixhawk.absolute_altitude_m, 0)
    await asyncio.sleep(takeoff_time - goto_time - takeoff_gap_time)
    kill_time = time.time()
...
async def main(takeoff_altitude, stay_arm_time, arm_time, takeoff_time, threshold, takeoff_gap_time):
    ...
    main_coroutines = [
        ...
        takeoff_and_kill(takeoff_altitude, stay_arm_time, arm_time, takeoff_time, takeoff_gap_time),
    ]
...
print("set threshold of light sensor")
threshold = int(input())
print("set stay_arm_time in float")
stay_arm_time = float(input())
...
print("set takeoff gap time in float")
takeoff_gap_time = float(input())

light = LightSensor()
pixhawk = Pixhawk()

aiorun.run(main(takeoff_altitude, stay_arm_time, arm_time, takeoff_time, threshold, takeoff_gap_time))
