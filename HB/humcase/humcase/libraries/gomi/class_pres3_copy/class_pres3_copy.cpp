...
bool pres::judge_alt_to_ground()
{
  ...
  */
  ...
  if (alt > ground_alt + minnimam_height + marge) 
  {
    if (judge_updown)
    {
      Serial.print("rising or falling:");
      Serial.println(alt);
      return false;
      //三番目にここ
    }
...
bool pres::judge_start_eeprom()
{
  ...
  */
  ...
  if (alt > ground_alt + eeprom_height + marge) 
  {
    if (is_eeprom_height)
    {
      return false;
    ...
    else
    {
      ...
      is_eeprom_height = true;
      Serial.println("is_eeprom_height true");
      return false;
      //2番目にここ
    }
  }
  ...
  else if (alt < ground_alt + eeprom_height - marge)
  {
    if (is_eeprom_height)
    {
      for (int i =0 ; i < check_times; i++)
