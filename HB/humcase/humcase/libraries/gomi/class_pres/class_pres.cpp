...
{
  set_pres(); //set_presも内部実装
  set_pres(); //set_presも内部実装
  set_pres();
  delay(1000);
  set_pres(); //最初の値はゴミの可能性
...
void pres::set_altitude_()
{
  set_pres();
  altitude_ =  ground_altitude_ + R * (ground_T + 273) * log(ground_pres/pressure) / g;
...
{
  float temp_altitude_ = 0.0;
  for (int j=0; j<3; j++)
  {
    delay(50);
    temp_altitude_ += get_altitude_();
  }
  pre_altitude_ = temp_altitude_ / 3;
  Serial.print("pre_altitude_");
  Serial.println(pre_altitude_);
}
