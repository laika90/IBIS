#include <class_pres5.h>
#include <EEPROM.h>

//staticの実態を定義
float pres::ground_pres;
float pres::ground_alt;
float pres::ground_T;
...
pres::pres()
{
  ...
  for(int j=0;j<20;j++){
    set_value1();
    alt_get = get_alt();
    sigma += alt_get*alt_get;
    y_k[0] += alt_get;
    }
  y_k[0] = y_k[0]/20;
  sigma = sigma/20-y_k[0]*y_k[0];
  sigma_k[0] = sigma/20;
  for(int j=1;j<l;j++){
    set_value1();
    alt_get = get_alt();
...
//  BME280測定条件設定(1回測定後、スリープモード)
  ...
  alt =  ground_alt + R * (ground_T + 273.15) * log(ground_pres/pressure) / g; //高度を計算
}
...
*/
{
  ...
  set_value1();
  alt_get = get_alt();
  alt_sus = 5.0*y_k[index[4]]-10.0*y_k[index[3]]+10.0*y_k[index[2]]-5.0*y_k[index[1]]+y_k[index[0]];
  sigma_sus = (25.0*sigma_k[index[4]]+100.0*sigma_k[index[3]]+100.0*sigma_k[index[2]]+25.0*sigma_k[index[1]]+sigma_k[index[0]]);
  sigma_k[index[0]] = 1/(1/sigma+1/sigma_sus);
  y_k[index[0]] = (sigma_sus*alt_get+sigma*alt_sus)/(sigma_sus+sigma);
  alt = y_k[index[0]];
  i2 = (i2+1)%l;
  delay(100);
}
...
bool pres::judge_alt_to_ground()
{
  ...
  */
  ...
  set_value();
  if (alt > ground_alt + minnimam_height + marge3) 
  {
    if (judge_updown)
    {
      return false;
  ...
  else if (alt < ground_alt + minnimam_height - marge4)
  {
    if (judge_updown)
    {
