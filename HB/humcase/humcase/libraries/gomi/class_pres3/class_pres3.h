...
*/
{
  ...
  const float minnimam_height = 1.5; //m
  const float marge1 = 1.5; //m、上昇判定の余裕
  const float marge2 = 1.5; //m、落下判定のための余裕
  const float marge = 0.5; //m、地面基準高度判定のための余裕
  const int check_times = 1; //追加でチェックする回数
  const int rising_time_ms = 1;
  //以上自分で設定する定数
